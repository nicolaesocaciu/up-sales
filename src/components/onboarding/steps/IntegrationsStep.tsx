...
export const IntegrationsStep = ({
  ...
  return <div className="flex-1">
      ...
      <div className="space-y-4">
        {integrations.map(integration => <IntegrationItem key={integration.id} id={integration.id} name={integration.name} description={integration.description} iconUrl={integration.iconUrl} connected={integration.connected} onToggle={() => toggleConnection(integration.id)} />)}
      </div>
    </div>;
};