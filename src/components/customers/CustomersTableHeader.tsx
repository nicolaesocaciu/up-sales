...
export const CustomersTableHeader = ({
  ...
  return <TableHeader className="bg-[#F2F2F2] rounded-[8px]">
      <TableRow className="hover:bg-transparent border-none h-12">
        ...
        {columnVisibility.amountSpent && <TableHead className="text-right cursor-pointer hover:bg-[#DADADA]" onClick={onSortChange}>
            <div className="flex items-center justify-end gap-2">
              Amount Spent
              {sortDirection === "asc" ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
            </div>
          </TableHead>}
        {columnVisibility.actions && <TableHead className="rounded-r-[8px] w-[50px] text-center">Action</TableHead>}
      </TableRow>
    </TableHeader>;
};