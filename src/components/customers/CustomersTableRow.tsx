...
const BuildingIcon = () => <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
...
export const CustomersTableRow = ({
  ...
  return <TableRow className="h-12 hover:bg-[#E7F2F9]">
      ...
      {columnVisibility.customerId && <TableCell>
          <span className="font-medium">#{customer.customerId}</span>
        </TableCell>}
      
      {columnVisibility.company && <TableCell>
      ...
      {columnVisibility.subscriptionStatus && <TableCell>
          {customer.subscriptionStatus === "subscribed" && <Badge className="rounded-[4px] px-3 bg-green-100 border-green-200 text-green-800 hover:bg-green-200">
              Subscribed
            </Badge>}
          {customer.subscriptionStatus === "not_subscribed" && <Badge className="rounded-[4px] px-3 bg-red-100 border-red-200 text-red-800 hover:bg-red-200">
              Not subscribed
            </Badge>}
          {customer.subscriptionStatus === "pending" && <Badge className="rounded-[4px] px-3 bg-yellow-100 border-yellow-200 text-yellow-800 hover:bg-yellow-200">
              Pending
            </Badge>}
        </TableCell>}
      
      {columnVisibility.location && <TableCell>{highlightText(customer.location)}</TableCell>}
      
      {columnVisibility.orders && <TableCell>
          {customer.orders === "N/A" ? "N/A" : `${customer.orders} Orders`}
        </TableCell>}
      
      {columnVisibility.amountSpent && <TableCell className="text-right">
          {customer.amountSpent === 0 && customer.orders === "N/A" ? "—" : formatCurrency(customer.amountSpent)}
        </TableCell>}
