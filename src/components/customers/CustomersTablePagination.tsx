...
interface CustomersTablePaginationProps {
  ...
  onCustomersPerPageChange: (value: string) => void;
}

export const CustomersTablePagination = ({
  totalCustomers,
  currentPageSize,
  currentPage,
  onPageChange,
  ...
  const startRange = ((currentPage - 1) * customersPerPage) + 1;
  const endRange = Math.min(startRange + currentPageSize - 1, totalCustomers);
  ...
  return (
    <div className="flex items-center justify-between px-4 py-4 border-t border-gray-200">
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <span>Showing {startRange} to {endRange} from {totalCustomers}</span>
      </div>
      <div className="flex items-center gap-2">
        ...
        <Select defaultValue="20" onValueChange={onCustomersPerPageChange}>
