...
interface MarketingCampaignsTablePaginationProps {
  totalCampaigns: number;
  currentPageSize: number;
  currentPage: number;
  onPageChange: (page: number) => void;
  campaignsPerPage: number;
  onCampaignsPerPageChange: (value: string) => void;
}

export const MarketingCampaignsTablePagination = ({
  totalCampaigns,
  currentPageSize,
  currentPage,
  onPageChange,
  campaignsPerPage,
  onCampaignsPerPageChange,
}: MarketingCampaignsTablePaginationProps) => {
  const totalPages = Math.ceil(totalCampaigns / campaignsPerPage);
  const startRange = ((currentPage - 1) * campaignsPerPage) + 1;
  const endRange = Math.min(startRange + currentPageSize - 1, totalCampaigns);
  ...
  return (
    <div className="flex items-center justify-between px-4 py-4 border-t border-gray-200">
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <span>Showing {startRange} to {endRange} from {totalCampaigns}</span>
      </div>
      <div className="flex items-center gap-2">
        ...
        <Select defaultValue="20" onValueChange={onCampaignsPerPageChange}>
